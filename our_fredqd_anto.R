...
setwd("C:/Users/anton/Desktop/fbi-master/R")

fredqd <- function(file,date_start,date_end,transform) {
 # Error checking
  ...
  attrdata <- read.csv('current.csv', header = FALSE, nrows = 3)
...
initial_data <- readr::read_csv('current.csv',col_names = FALSE,cols(X1 = col_date(format = "%m/%d/%Y")),skip = 3)

attrdata <- read.csv('current.csv', header = FALSE, nrows = 3)
header <- c("date", unlist(attrdata[1,2:ncol(attrdata)]))
colnames(initial_data) <- header

colsum <- colSums(is.na(initial_data))
which(colsum >= 38)
